...
void MainWindow::on_connecter_clicked()
{
    ...
    uint8_t data[16];
    int status1 = Mf_Classic_Read_Block(&MonLecteur, TRUE, 10, data, Auth_KeyA, 2);
    QString nom = QString::fromStdString(std::string((char*)data));
    int status3 = Mf_Classic_Read_Block(&MonLecteur, TRUE, 9, data, Auth_KeyA, 2);
...
void MainWindow::on_pushModif_clicked()
{
    ...
    Mf_Classic_Write_Block(&MonLecteur, TRUE, 10, newName, Auth_KeyB, 2); // Modification nom
    ...
    Mf_Classic_Write_Block(&MonLecteur, TRUE, 9, newPrenom, Auth_KeyB, 2); // Modification prenom
...
void MainWindow::on_pushButton_formater_clicked()
{
    ...
    Mf_Classic_Write_Block(&MonLecteur, TRUE, 10, newName, Auth_KeyB, 2); // RAZ nom
    const QString b = "";
    sprintf(dataIn, a.toUtf8().data(), 16);
    uint8_t newPrenom[16];
    for (int i = 0 ; i < 15 ; i++) {
    ...
    Mf_Classic_Write_Block(&MonLecteur, TRUE, 9, newPrenom, Auth_KeyB, 2); // RAZ prenom
...
{
    char DataIn[16]="ftes";
    sprintf(DataIn, ui->sortie->toPlainText().toUtf8().data(), 16);
